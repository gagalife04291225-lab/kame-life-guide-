...
- **ASINと商品名の突き合わせ**（`data/products.js`）：ASINが有効でも、**サイトに書いてある商品名がASINの実商品と違っている**ことがある。リンクは生きているため、リンク切れチェックでは検出できない
  - 手順：各商品の `asin` を `"<ASIN>" amazon.co.jp` で `web_search` し、返ってきた商品ページのタイトルと `name` を突き合わせる
  - 見るのは名称の一致だけではない。**購入時に効く属性**（電源周波数・容量・サイズ・入り数・口金・測定範囲）が名称や `why` から抜けていないかも確認する
  - `name` が「水槽 60cm規格」のような汎用名になっているものは、実商品名に置き換える
  - 検索でASINを特定できなかったものは**推測で書き換えず「未確認」のまま残す**
  - 実績：2026-08-04 に81件中69件を照合。名称の誤り3件（別商品・誤訳・サプリとフードの取り違え）、汎用名8件、周波数の記載漏れ1件を是正。詳細は該当コミットのメッセージに記録
