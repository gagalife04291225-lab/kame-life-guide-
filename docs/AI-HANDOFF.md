...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #64**（C分類22件の置換）を含む `origin/main` |
...
- 処理済 147件（B-1 37 + B-2 37 + A-1 26 + A-4 5 + H1 14 + A-5 3 + B18 1 + H2 2 + C 22）→ **残 92件**
...
- **C分類（おすすめ・選び方）22件は完了・未処理0。** 「初心者におすすめ／人気／飼いやすい」は
  ...
  **C分類はクローズ。再監査しない**
- **残る未処理分類は D（注意・警告）24件 / E（見出し・CTA・ナビ）30件 /
  F（その他）12件 / A分類の残り**。合計92件
...
### D分類24件 — 「初心者には難しい」を、難しさの理由を名指しする文へ書き換える

**なぜ今これか**: C分類が閉じ、残る未処理は D 24 / E 30 / F 12 / A の残り。
D は Phase A で「**機械置換禁止・1件ずつ根拠取りが必要**」と判定された唯一の分類で、
最も手間がかかる。他分類より先に片づけないと、後から着手コストが上がる。

| 項目 | 内容 |
|------|------|
| **対象** | Phase A の **D分類 24件 / 22ファイル**。※着手時に現mainから実数を再抽出し、24件と一致しなければ STOP して差異を報告 |
| **Scope** | 「初心者には難しい」「上級種のため初心者が最初の1頭として選ぶには難易度が高い」等の警告文。**「入門向けではない」への機械置換は禁止**（難易度ラベルは別欄に既にあり、文章側の役割は理由の提示）。Phase A の難所5型（① 湿度・温度の同時管理 ② 汽水・塩分管理 ③ 給餌の難しさ ④ 終生サイズ・スペース ⑤ 情報・流通の乏しさ）へ各種を割り当ててから書く |
| **根拠取り** | 各種の難所は**そのページの本文・`species-master.json`・`SHINDAN-SPECIES.md` から取る**。推測で難所を作らない。根拠が見つからない種はその1件だけ HOLD |
| **開始前ゲート** | 24件それぞれで JSON-LD複製 / 属性値 / 見出し・パンくず / `title`・`meta` を機械判定。複製があれば可視と同一commitで同期（PR #59 / #60 / #64 の手順）。該当するものだけ HOLD |
| **変更禁止** | **警告の強さを弱めないこと**／difficulty の値・★・色・アイコン（D分類の文と難易度ラベルは別物）／A-1「飼育難易度」・A-4「はじめて飼う亀に向きますか？」・A-5「はじめての1頭に向くか」／B・C・E・F分類／CTA・ナビ／`title`・`meta`・OG・canonical／`href`・URL・slug／`sort=beginner`／`?diff=初心者向け`／GA4／`s.beginner`／`data-*`／taxonomy／写真／CITES／学名・和名／`shindan/species.js`／複合質問2件／`compare/hermann-vs-greek.html` の3件 |
| **完了条件** | ①D分類24件の置換完了（HOLD分を除く）②各件で難しさの理由が具体的に書かれている（「入門向けではない」だけで終わっていない）③警告の強さが変わっていないことを1件ずつ確認 ④JSON-LD複製があった分は可視と完全一致・パース成功 ⑤Scope Lock 対象に意図外差分0 ⑥generator 4本 `--check` 差分0 ⑦JSエラー増加0 ⑧commit → push → PR → merge → main反映確認 ⑨本ファイルの更新 |
