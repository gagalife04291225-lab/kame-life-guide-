...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #72**（E-2 shindan の関連リンクカード15行）を含む `origin/main` |
...
### E-3 `shindan/index.html` に残る可視の「初心者」6行 — バッジ・生成文・reason

**なぜ今これか**: E-2（PR #72）で関連リンクカードを閉じたので、`shindan/index.html` に残る
「初心者」は14件。うち**8件は `//` `<!--` のコメントで画面に出ない**ため対象外。
**可視は6行だけ**で、これを閉じれば `shindan/` 配下の公開表示から「初心者」が消える。
H5 / H6 / U1 / U2 はいずれも**亀好きさんの判断待ち**、N3 の残り62行は方針未定なので、
判断待ちを増やさずに進められるのはこの1工程。

| 項目 | 内容 |
|------|------|
| **対象** | **6行**。※着手時に現mainから再抽出し、6行と一致しなければ変更せず STOP して差異を報告<br>**バッジ配列2件** `:1394`（`semi_aquatic_small` の `'🎯 初心者向き'`）／`:1400`（`BARRIER_BADGE_BEGINNER` の `'🎯 初心者向き'`）<br>**生成文2件** `:1450`（`clauses.push('初心者向けの回答内容に合わせやすい、管理しやすい入門種です。')`）／`:1567`（`return '初心者向けの回答に対して、他候補より始めやすい候補です。'`）<br>**`reason:` 2件** `:2668`（exotic ルートの安全 fallback「…ミシシッピニオイガメは初心者に最も適した定番種です。」）／`:2860`（「初心者に最も適した水棲ガメです。…」） |
| **Scope** | 可視文字列のみ。確定済み語彙（`はじめての方向け` / `はじめて飼う方` / `はじめての1頭` / `はじめてでも`）から選び、**新語を作らない**。バッジ2件は**同じ文字列なので同じ語に揃える**。`reason` 2件は**推薦の強さ（「最も適した」）を弱めない・強めない** |
| **絶対に変更禁止** | **変数名・キー名**（`BARRIER_BADGE_BEGINNER` / `semi_aquatic_small` / `BEGINNER_WARN_DATA` など。`beginner` を含む識別子は互換のため変更しない）／バッジ配列の**要素数・順序**／`clauses.push` の**呼び出し条件・分岐**／`reason` を持つオブジェクトの他のキー（`slug` / `name` / `difficulty` 等）／診断ロジック・採点・ルート定義 |
| **同ファイル内で触らないもの** | コメント8件（`:346` `:1175` `:1424` `:1704` `:1751` `:2654` `:2656` `:3754`）。**画面に出ないため対象外**。今回も将来も、公開表現の工程では扱わない |
| **その他の変更禁止** | H5 / H6 の9件（判断待ち）／`shindan/species.js`・`routes.js`・`equipment.js`／difficulty の値・★・色・アイコン／確定済み文言（A-1 `飼育難易度` / A-4 / A-5 / B・C・D・F の全確定文 / annainin の `はじめての方向け` 系4箇所 / PR #70 の compare 6件 / PR #71 のスコア表5行 / PR #72 の関連リンクカード15行）／`title`・`meta`・OG・canonical／`href`・URL・slug／GA4イベント名・パラメータ／`data-*`／`id`・`class`／taxonomy／写真／CITES／学名・和名 |
| **開始前ゲート** | 6行それぞれで ①その文字列が**条件分岐・比較（`indexOf` / `===` など）に使われていないこと** ②GA4 パラメータに使われていないこと ③`href` に使われていないこと ④JSON-LD複製がないこと を機械確認する |
| **完了条件** | ①6行の置換完了（同じ文字列は同じ語）②バッジ配列の要素数・順序に差分0 ③5ルートを実描画し、**バッジの表示件数・`reason` の表示位置が変更前と一致** ④結果画面の遷移先（`href`）が変更前と一致 ⑤`shindan/index.html` の「初心者」残存が**コメント8件のみ**になること ⑥Scope Lock 対象に意図外差分0 ⑦JSエラー増加0 ⑧generator 4本 `--check` 差分0 ⑨commit → push → PR → merge → main反映確認 ⑩本ファイルの更新 |
