...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #75**（N5 解消 — js/ 配下の共有スクリプト8件）を含む `origin/main` |
...
### （実行可能な工程なし）— 判断待ち4件の決定を仰ぐ

**PR #75 をもって、亀好きさんの判断なしに進められる「初心者」表現の置換は尽きた。**
`shindan/` は公開表示0件、`js/` は非表示キーワード1件のみ。
残っているものはすべて **① 判断待ち ② SEO と不可分 ③ 変更しないと決めたもの** のいずれかで、
**Claude Code 側だけで着手できる工程は存在しない。**

**次のプロンプトで新しい置換工程を指示しないこと。** まず下の4件を決める。

| 現状の残存（main `922a729` 時点の実測・公開HTML 123行 ＋ `js/` 1行） | 件数 | 状況 |
|---|---|---|
| `title` / `meta` / OG / JSON-LD の `headline`・`description`・`name` | 60 | **SEO②。運営フェーズの方針では GSC/GA4 の実測がトリガーを引いたページのみ・週最大3ページ**。一斉置換は方針違反 |
| 見出し（h1-h6）9 ＋ パンくず 2 | 11 | 上の SEO と同時でないと `title` と `h1` が乖離する |
| 本文・ラベル | 38 | うち **H5 6件 / H6 3件が判断待ち**、残り29件は A/C/D/E のどれか文脈判断が要る（`N3`） |
| `href` / `?diff=初心者向け` / `<option value="beginner">` / GA4 `page_title` | 4 | **互換のため恒久的に変更禁止** |
| コメント（`shindan/index.html` 8 ＋ 他2） | 10 | 画面に出ない。恒久的に対象外 |
| `js/annainin.js:127` の `INTENT_RULES` | 1 | 非表示の分類キーワード。恒久的に変更禁止 |

**亀好きさんに決めてほしい4件（`UNRESOLVED` の詳細を参照）**

1. **U1** — badge★を難易度と1対1にするか（`中級★3 / 中〜上級★4 / 上級★5`）。ラベルが既に正しい71ページに波及する
2. **U2** — 「初心者」を公開表現に使わない方針を `CLAUDE.md` に明記するか
3. **H5** — 同じ表・カードに難易度軸が2本ある6件を、統合するか初心者軸を別名で残すか
4. **H6** — 勝者バッジ / hero-stat / カードタグの3件を C・E どちらの扱いにするか

**あわせて検討したいこと**: 本プロジェクトは運営・改善フェーズにあり、`docs/operations/DECISION_RULE.md` は
**実測値がトリガーを引いたページのみを改善する**と定めている。「初心者」表現の置換は
実測トリガーではなく内部の一貫性のための作業で、サイトの中核（診断ツール・比較表・
ランキング・機材CTA・species ページ）はすでに完了している。
**次の一手として、実際に発生した購入の導線（GA4 の `buyer_guide_click` / `exit_capture_click` /
アフィリエイトレポート）を見て、改善対象を数字で決める**ほうが優先度が高い可能性がある。

> **UNRESOLVED を勝手に全部処理しない。** H3 は再探索禁止、`N3` の29行は方針未定。
> **判断が示されるまで、新しい置換工程を自発的に開始しない。**
