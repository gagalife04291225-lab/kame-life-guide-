...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #74**（N4 解消 — species.js の reason 3件）を含む `origin/main` |
...
| ID | 内容 |
|----|------|
| N5 | **`js/` 配下の共有スクリプトに公開表示される「初心者」が8件ある**（PR #74 で発見）。`js/quick-facts.js` 2（CTAラベル・**111ページが読み込む**）／`js/starter-kit.js` 2（tierバッジ `🔰 初心者向け`・**113ページ**）／`js/comparison-cta.js` 2（比較ポイント配列・**12ページ**）／`js/ranking-engine.js` 2（ランキング名。読み込むページは**0**）。**Phase A の239件は公開HTMLのみが対象で、`js/` 配下は集計に入っていない**（N2 と同種の漏れ）。`js/annainin.js:127` の `INTENT_RULES` 1件は非表示の分類キーワードで**恒久的に対象外**。→ 下記 NEXT で処理する |
...
### N5 解消 — `js/` 配下の共有スクリプトの公開表示「初心者」8件

**なぜ今これか**: `shindan/` は PR #74 で公開表示0件になった。一方で **`js/` 配下の
共有スクリプトに未処理の公開表示が8件残っている**（PR #74 で発見）。
うち `quick-facts.js` は **111ページ**、`starter-kit.js` は **113ページ**が読み込むため、
**1ファイル直すと100ページ超に一度に反映される**影響範囲の大きい塊。
H5 / H6 / U1 / U2 は**亀好きさんの判断待ち**、N3 の残り38行は方針未定なので、
判断待ちを増やさずに進められるのはこの1工程。

| 項目 | 内容 |
|------|------|
| **対象** | **8件 / 4ファイル**。※着手時に現mainから再抽出し、8件と一致しなければ変更せず STOP して差異を報告<br>`js/quick-facts.js:182` `'初心者向けに全部選んだ飼育セットを見る'`／`:183` `'初心者向け飼育セットを見る'`（CTAラベル・111ページ）<br>`js/starter-kit.js:366` `:373` `{ icon: '🔰', label: '初心者向け' }`（tierバッジ・113ページ・**同じ文字列なので同じ語に揃える**）<br>`js/comparison-cta.js:23` `:39` `points` 配列の `'初心者向け'`（12ページ・**同じ文字列**）<br>`js/ranking-engine.js:18` `'初心者向け総合ランキング'`／`:90` `'初心者向けリクガメランキング'`（読み込むページは**0**だが公開ファイルなので揃える） |
| **Scope** | 可視ラベルのみ。確定済み語彙（`はじめての方向け` / `はじめて飼う方` / `はじめてでも` / `はじめての1頭`）から選び、**新語を作らない**。`🔰` アイコンは維持。CTAの誘導強度・ランキング名の意味は変えない |
| **★着手前の安全ゲート（必須）** | 8件それぞれで ①その文字列が**条件分岐・比較（`indexOf` / `===` / `includes` / `switch`）に使われていないこと** ②GA4 パラメータ・イベント名に使われていないこと ③`href`・URL に使われていないこと ④`label` / `points` が**キーとして参照されていないこと**（例: `POINTS['初心者向け']`）を機械確認する。使われていたら**その件だけ HOLD** |
| **絶対に変更禁止** | `js/annainin.js:127` の `INTENT_RULES` beginner キーワード（**非表示の分類語彙・恒久的に変更禁止**）／変数名・キー名・`icon` の絵文字／配列の順序・件数／`tier` の判定条件（`=== 'budget'` 等）／GA4イベント名・パラメータ |
| **その他の変更禁止** | H5 / H6 の9件（判断待ち）／difficulty の値・★・色・アイコン／確定済み文言（A-1 `飼育難易度` / A-4 / A-5 / B・C・D・F の全確定文 / annainin の `はじめての方向け` 系4箇所 / PR #70 の compare 6件 / PR #71 のスコア表5行 / PR #72 の関連リンクカード15行 / PR #73 の可視6行 / PR #74 の reason 3件）／`shindan/` 配下（PR #73 / #74 で確定済み）／`title`・`meta`・OG・canonical・JSON-LD／`href`・URL・slug／`data-*`／`id`・`class`／taxonomy／写真／CITES／学名・和名 |
| **完了条件** | ①8件の置換完了（HOLD分は理由つきで報告）②変数名・キー名・配列の順序と件数に差分0 ③`quick-facts.js` / `starter-kit.js` / `comparison-cta.js` を読み込む代表ページを実描画し、**CTAラベル・tierバッジ・比較ポイントが新しい語で表示され、リンク先（`href`）と表示件数が変更前と一致**すること ④JSエラー増加0 ⑤generator 4本 `--check` 差分0 ⑥`js/` 配下の公開表示「初心者」が **`annainin.js:127` の1件のみ**になること ⑦commit → push → PR → merge → main反映確認 ⑧本ファイルの更新 |

> **UNRESOLVED を勝手に全部処理しない。** 今回実行するのは上記1工程だけ。
> **U1 / U2 / H5 / H6 は亀好きさんの判断待ち**、H3 は再探索禁止、N3 の残り38行は方針未定のため触れない。
