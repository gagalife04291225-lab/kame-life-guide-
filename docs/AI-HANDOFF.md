...
| 項目 | 値 |
|------|-----|
| 基準 | `origin/main` = **4e2b584**（PR #99 = species-list のデザイン実装1 ＋ 法規制表示の回帰修正 の merge commit まで反映済み） |
| サイトファイルの状態 | **PR #99 = `872cdfd`** が最後。**merge 待ち = 本PR（species-list の掲載区分 通常115＋参考3）** |
| 確認方法 | `git log --oneline -1 origin/main` で**実測する**。表の値は下記の理由で常に1〜2 commit 遅れる |
| 最終更新日 | 2026-08-28 |
| 作業ブランチ | `claude/kame-life-species-list-listing-split` |
| 作業ツリー | clean（`origin/main` ＋ 本PRの変更のみ） |
| 本PRの変更 | `species-list.html` ／ `tools/taxonomy.js` ／ `tools/gen-species-list.js` ／ `tools/gen-category-ui.js` ／ `docs/AI-HANDOFF.md` |
...
| 工程 | 作業 | 状態 | 確定した結論 |
|------|------|------|-------------|
| 掲載区分の実装 | 参考データを通常一覧から分離 | 本PR | 5ファイル。**通常一覧115 / 参考掲載3 / 合計118**。`shindan/species.js` は1バイトも触っていない |
...
### ② species-list.html —「暮らしから選ぶ」6大分類の写真タイル

一覧そのものは実装1で軽くなり、掲載区分も本PRで片づいた。**どちらも再実装・再測定しない。**
監査で設計した新構成のうち、**まだ入っていないのは写真タイルだけ**。

| 項目 | 内容 |
|------|------|
| **やること** | 操作バーと一覧の間に、6大分類の写真タイル（2列×3行）を置く。タップでその分類に絞り込む（既存の `#hab-btns` の状態を動かすだけで、フィルタのロジックは足さない） |
| **なぜこれが次か** | 現在 6大分類は操作バーの横スクロールチップだけで、写真で選ぶ入口がない。図鑑としての第一印象はここで決まる |
| **タイルに出す種数** | **通常一覧基準で 25 / 21 / 18 / 29 / 6 / 16 ＝ 115**。参考掲載3種は数に入れない。値は `tools/gen-category-ui.js` のログで実測できる |
| **タイルの写真** | **新規撮影・生成はしない。** 既存の `assets/species-photos/*.webp`（107枚）から各分類の代表種1枚を流用する。どの種を代表にするかは実装時に決めて報告する |
| **対象** | `species-list.html` のみ（種数を焼き込むなら `tools/gen-category-ui.js` も） |
| **変更禁止** | 生成領域4つ（`hab-buttons` / `taxonomy-data` / `wamei-alias` / `species-index`）の手編集 ／ `title`・`meta`・OG・twitter・JSON-LD（H8）／ `shindan/species.js`・`equipment.js`・`CAT_OVERRIDE` の和名3点連結 ／ `index.html` ／ 診断ロジック ／ 総掲載「118種」表記 ／ 掲載区分（`REFERENCE_ONLY`）／ legal 14種のラベル ／ N22・N23・N24・N27・N28・U-D・H3・H8・H9 |
| **完了条件** | 390px の全長が **21,000px を超えない**（現状 19,837px。タイル追加ぶんの増加を抑える）／ タイルから絞り込みが効く／ 通常115・参考3・合計118と6分類の件数が正本と一致／ 検索・絞り込み・並び替えが従来どおり／ JSエラー0 ／ 横スクロールなし ／ generator 4本 `--check` 差分0 ／ SEO層・JSON-LD 差分0 ／ `docs/AI-HANDOFF.md` を同期 |

**新しいアイコンセットは作らない。** 分類の識別は写真タイルとテキストで足りる。
