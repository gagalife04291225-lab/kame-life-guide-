...
| 項目 | 値 |
|------|-----|
| 基準 | `origin/main` = **e63ff43**（PR #144 merge・2026-09-07 実測値） |
...
- **N31 鼻水ページが2枚あり、相互リンクもなく重複している。** 第3弾の作業中に判明した。
  `guides/turtle-runny-nose.html`（GSC 168表示/8.29位）と `trouble/runny-nose.html`（480行）が
  **どちらも `鼻水` を主題**とし、**両方が sitemap.xml に掲載**され、
  **それぞれ自分自身を canonical** にしている。作業前の相互リンクは **0本**。
  - 差別化の実測: 「くしゃみ」の出現数は guides 側 **12回** / trouble 側 **0回**。
    trouble 側は `鼻詰まり`・URI・肺炎リスクが主題
  - **今回は統合していない。** guides 側を「くしゃみ・鼻水の判断」へ寄せ、
    trouble 側へ文脈付きリンクを1本追加したのみ（**trouble 側のファイルは無変更**）
  - **canonical の統合・sitemap からの削除は Owner 判断事項。**
    GitHub Pages では `_redirects` が効かないため canonical 書き換えが実質的な手段になる。
    **勝手に実施しない**
...
> 先に着手すべき候補があるとすれば **N31（鼻水ページ2枚の重複解消）**だが、
> これは Owner 判断事項であり、勝手に統合しない。
