...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #66**（F分類11件の置換 / F3-2 は HOLD）を含む `origin/main` |
...
- 処理済 182件（B-1 37 + B-2 37 + A-1 26 + A-4 5 + H1 14 + A-5 3 + B18 1 + H2 2 + C 22 + D 24 + F 11）→ **残 57件**
...
- **D分類（注意・警告）24件は完了・未処理0。** 抽象的な「初心者には難しい」を
  ...
  **D分類はクローズ。再監査しない**
- **F分類（その他）12件は 11件完了 / HOLD 1件。** 読者層の指定・採点基準の説明・UI例文を
  「はじめて飼う方」「飼育経験が浅いうちは」系へ置換済み。**F分類はクローズ**（HOLD 1件を除き再監査しない）
- **残る未処理分類は E（見出し・CTA・ナビ）30件 / A分類の残り**。合計56件 ＋ F の HOLD 1件 = 57件
...
#### `annainin`（亀の案内人チャット）の実測構造 — PR #66 で確定・**再調査しない**

- `js/annainin.js:347-349` のハンドラは `getAttribute('data-quick-reply')` **だけ**を読む。
  `.an-topic-btn` の可視ラベルは**一切読まれない** → 可視ラベルを変えても挙動は変わらない
- ただし `submitUserText()` は送信テキストを**そのままユーザー発言バブルとして画面に描画**する。
  よって可視ラベルだけ変えると「クリックした語」と「画面に出る自分の発言」が食い違う
- `js/annainin.js:151` `TOPIC_QUICK_REPLIES` の `初心者向けの亀は？` は、`:405-408` で
  `data-quick-reply` と `textContent` の**両方**に同じ文字列が入る。ボットの返信ボタンは
  `初心者向けの亀は？` を**可視表示**している
- `INTENT_RULES` の `beginner` キーワードは `['初心者','はじめて','初めて','入門']`。
  **`はじめて` は既に beginner に分類される**ため、例文を「はじめて」系にしても意図分類は変わらない
  （PR #66 の実描画テストで、新旧プレースホルダ例文がともに同一の bot 応答へ到達することを確認済み）
...
| H4 | **F3-2 `annainin/index.html:74` の topic-btn 可視ラベル「初心者向け」。** 可視だけ変えると、クリック後に描画される自分の発言が `初心者向けの亀は？` のままになり、押した語と食い違う。解消には内部値 `data-quick-reply`（変更禁止）と `js/annainin.js` の `TOPIC_QUICK_REPLIES`（F の Scope 外）を同時に変える必要がある。**annainin の語彙統一として1工程を別に立てるまで着手しない** |

### 新発見（未処理）

| ID | 内容 |
|----|------|
| N2 | **`js/annainin.js` に公開表示される「初心者」が4箇所ある**（`:151` `TOPIC_QUICK_REPLIES` の1件はボットの返信ボタンとして可視、`:188` `:314` の2件は bot 応答本文、`:127` の1件は意図分類キーワードで非表示）。Phase A の集計は公開HTMLのみを対象にしていたため **239件には含まれていない**。`:127` のキーワードは削除すると「初心者」と入力したユーザーを分類できなくなるため**残す**。H4 と同じ工程で扱うのが自然 |

---

## NEXT — 次に実行する工程（**1つだけ**）

### annainin の語彙統一（H4 + N2）— `data-quick-reply` と `js/annainin.js` を含めて1回で閉じる

**なぜ今これか**: F分類は11/12まで閉じたが、残る1件（H4）は可視ラベルだけでは直せない。
原因は PR #66 で実測済みで、`annainin/index.html` の内部値1件と `js/annainin.js` の可視文字列3件が
同じ語彙に縛られていること。**ここを1回で揃えないと、annainin だけ「初心者」が残り続ける。**
E分類の残り30件は h1・パンくず・ページ主題見出しが中心で、Phase A の判断どおり
SEO（②87件）と同時でないと `title` と `h1` が乖離するため、まだ着手しない。

| 項目 | 内容 |
|------|------|
| **対象** | ①`annainin/index.html:74` の可視ラベル「初心者向け」と内部値 `data-quick-reply="初心者向けの亀は？"`（**今回に限り内部値の変更を可とする**）②`js/annainin.js:151` `TOPIC_QUICK_REPLIES` の `初心者向けの亀は？`（可視）③`js/annainin.js:188` `:314` の bot 応答本文2件（可視）。**計4箇所**。※着手時に現mainから実数を再抽出し、4箇所と一致しなければ STOP して差異を報告 |
| **Scope** | 「初心者向け」→「はじめての方向け」系へ、**可視ラベル・内部値・`TOPIC_QUICK_REPLIES`・bot応答本文が同じ語彙になるよう同一commitで揃える**。文の構造・語順・敬体は変えない |
| **絶対に変更禁止** | `js/annainin.js:127` の `INTENT_RULES` beginner キーワード配列 `['初心者','はじめて','初めて','入門']`。**`初心者` を削ると「初心者」と入力したユーザーが `unknown` に落ちる。** 分類キーワードは公開表示ではないため、そのまま残す |
| **その他の変更禁止** | difficulty の値・★・色・アイコン／A・B・C・D・E・F 分類の確定済み文言／CTA・ナビ／`title`・`meta`・OG・canonical／`href`・URL・slug／GA4イベント名・パラメータ（`annainin_view` / `annainin_message_sent` / `annainin_intent_classified`）／`id`・`class`／他の9つの topic-btn／`shindan/` 配下 |
| **開始前ゲート** | 4箇所それぞれについて、①その文字列が意図分類・GA4・`href` に使われていないこと ②JSON-LD複製がないこと を機械確認する |
| **完了条件** | ①4箇所すべてが同じ語彙で一致 ②`.an-topic-btn` をクリックしたとき、**押した可視ラベルと描画されるユーザー発言が矛盾しない** ③クリック後の bot 応答が変更前と同一（intent が `beginner` のまま）④プレースホルダ例文を入力送信した場合も bot 応答が同一 ⑤JSエラー増加0 ⑥generator 4本 `--check` 差分0 ⑦commit → push → PR → merge → main反映確認 ⑧本ファイルの更新 |
