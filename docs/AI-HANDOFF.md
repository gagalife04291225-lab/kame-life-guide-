...
| 項目 | 値 |
|------|-----|
| 基準 | `origin/main` = **803b8b4**（PR #145 merge・2026-09-07 実測値） |
...
### N31 鼻水ページ2枚の重複 — 比較分析（2026-09-07 / 本PR・**分析のみ。統合は未実施**）

**結論: 暫定判定は KEEP BOTH（統合しない）。ただし最終確定は STOP。**
`trouble/runny-nose.html` の GSC 実測値が存在しないため、
「GSC を見ずに統合しない」という指示に従い canonical 統合・MERGE は実行していない。
**サイトファイルは無変更**（本PRはドキュメントのみ）。

**実測した2ページの差**

| 項目 | `guides/turtle-runny-nose.html` | `trouble/runny-nose.html` |
|---|---|---|
| GSC | **168表示 / 3クリック / CTR1.79% / 8.29位** | **データなし（未取得）** |
...
1. 本文の実重複が低い（Jaccard 0.086）→ **MERGE の必要性が薄い**
2. 固有語が明確に分離しており、役割を分けて運用できる → **KEEP BOTH が成立しうる**
3. `canonical` 統合は事実上**不可逆**（統合先に評価が移り、統合元はインデックスから外れる）。
   `trouble` 側の表示・クリックが不明なまま実行すると、**取れている流入を失う可能性を検証できない**
4. GitHub Pages では `_redirects` が効かないため、統合手段は canonical 書き換えに限られ、
   やり直しコストが高い

→ **不可逆な操作を、判断材料が欠けたまま実行しない。**
...
### N31 の比較実測値（2026-09-07 / 再測定しない）

上表の全数値は実測済み。**同じ比較を再実行しない。**
不足しているのは **`trouble/runny-nose.html` の GSC のみ**。

**受領後の判定ルール（Owner が GSC を取得したら、この分岐で決める）**

| `trouble/runny-nose.html` の実測 | 判定 | 実施内容 |
|---|---|---|
| 「亀 鼻水」等で**表示を獲得している**（目安: 対象期間で表示20以上） | **KEEP BOTH** | 両ページの重複3セクション（よくある原因／自宅でできる確認／受診サイン）を役割に沿って削り分け、相互リンクを双方向にする。canonical は変更しない |
| **表示がほぼ0**（目安: 表示5未満）かつ guides 側が同クエリで上位 | **CANONICAL** | `trouble/runny-nose.html` の canonical を `guides/turtle-runny-nose.html` へ向け、sitemap から trouble 側を外す。固有の緊急度マトリクスは guides 側へ移設してから実施する |
| 同一クエリで**両URLが入れ替わって出ている** | **MERGE** | trouble 側の緊急度マトリクスと URI・肺炎の記述を guides 側へ統合し、上と同じく canonical を向ける |

**どの分岐でも守ること**: `_redirects` は GitHub Pages で効かないため使わない。
統合する場合は**先に内容を移してから** canonical を変更する（内容を失わせない）。
...
- **N31 鼻水ページ2枚の重複 — 比較分析は完了。判定は `trouble` 側の GSC 待ちで STOP。**
  2ページの内容差・重複率・被リンク・schema は**すべて実測済み**（COMPLETED / FIXED_FACTS 参照）。
  **再比較しない。**
  - 暫定判定は **KEEP BOTH**（本文の実重複が Jaccard 0.086 と低く、固有語も分離しているため）
  - **不足しているのは `trouble/runny-nose.html` の GSC（clicks / impressions / CTR / position / query）だけ。**
    受領したら FIXED_FACTS の分岐表どおりに判定して実施する
  - canonical 統合は不可逆なため、データが揃うまで実行しない

- **N30 難易度・寿命が資料間で食い違う。** 今回の比較表作成中に判明した。
  `SHINDAN-SPECIES.md`（正本とされる参照表）と `shindan/species.js`（診断ツールの実データ）で
  難易度が **3件**食い違う: ヒメハコヨコクビガメ（md 上級 / js 中級）、
  カブトニオイガメ（md 入門 / js 入門〜中級）、ロシアリクガメ（md 中級 / js 入門〜中級）。
  寿命も ミシシッピニオイガメで 種ページ「20〜30年以上」と js「20〜50年」が食い違う。
  **今回は表示の一貫性を優先し、難易度＝`shindan/species.js`、寿命＝種ページ表記に統一した**
  （比較表のリンク先＝種ページと表示が矛盾しないことを優先）。
  **どちらを正本とするかは未確定。ページ側を直す前に正本を決める必要がある。**
...
>
> **SEO改善は第1弾（サイズ軸 / small-best-turtles）・第2弾（飼いやすさ軸 / ranking-beginner-top10）
> ともに完了した（2026-09-07）。どちらも再実行しない。**
> **第3弾「亀 くしゃみ・鼻水」も完了した（2026-09-07）。再実行しない。**
> SEO改善は第1弾（サイズ軸）・第2弾（飼いやすさ軸）・第3弾（症状の判断軸）まで完了。
> 次工程は**3本の GSC 実測（順位・CTR の変化）を確認してから**決める。
> **N31 の比較分析は完了した（2026-09-07）。再比較しない。**
> 不足は `trouble/runny-nose.html` の GSC のみ。受領したら FIXED_FACTS の分岐表で判定する。
