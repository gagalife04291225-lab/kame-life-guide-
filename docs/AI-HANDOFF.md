...
### Owner 裁定 D-01/D-02/D-03 の規則反映（2026-09-03 / 本PR）
...
| D-03 | 裁定 D-01(案B) 以前に Actor が merge した3件（PR #129/#130/#131）の扱い | 手続き違反が記録に残ったまま |

**裁定済み（2026-09-03）**: 旧 D-01 Merge 権限 → **案B**（Actor は merge しない）／
旧 D-02 品質不足の STOP → **案A**（`closeout-gate.md` に追加済み）／
旧 D-03 基準の引き下げと検査の追加の区別 → **案A**（`pm-conduct.md` §4 に追加済み）
