...
| D-05 | **kame ゲート K1（0秒の中央被写体）の判定式が実物と合わない**（案A: 判定領域を縦構図に合わせて**較正のうえ**変更 / B: 宣言＋目視へ退避 / C: 閾値引き下げ＝**不可**） | kame の動画が K1 で落ち続ける |
| D-06 | **kame で G2（冒頭10秒が無音でない）を維持するか**（A: 維持 / B: 非適用にし K8「音声の出所を宣言」を新設 / C: 単純撤廃＝**不可**） | **PLAYBOOK⑦が公開できない** |

**裁定済み（2026-09-04）**: D-04 SHORTS GATE のジャンル別プロファイル化 → **案A**（実装済み・science 回帰 PASS）／
