...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #76**（最終フェーズ U2 + H5 + H6 + N3。表記整理 CLOSED）を含む `origin/main` |
...
### 「初心者」表記整理は CLOSED — 次は実測データにもとづく運営・改善

**PR #76 をもって「初心者」表記整理プロジェクトを CLOSED とした。**
公開HTMLに残る97行は全件が SEO運用HOLD / 内部互換 / 明示HOLD / 非表示コメント で説明でき、
**説明不能な公開表示は0件**。判断待ちも0件。

> **新しい「初心者」表記調査フェーズを作らない。**
> 全件走査・再分類・件数の再確認をやり直さない。
> 残存97行の内訳は `FIXED_FACTS` が正本で、これを固定入力として扱う。
> SEO系80行は `H8` として恒久HOLD。**能動的に着手しない。**

**次にやること**: 本プロジェクトは運営・改善フェーズにあり、
`docs/operations/DECISION_RULE.md` は **実測値がトリガーを引いたページのみを改善する
（週最大3ページ）** と定めている。表記整理は実測トリガーではなく内部の一貫性のための
作業で、それが終わった今、**次の一手は実測を見て決める**のが方針どおり。

| 手順 | 内容 |
|------|------|
| **① 実測を集める** | GA4 の `buyer_guide_click` / `exit_capture_click` / `annainin_intent_classified`、Search Console の表示回数・CTR・掲載順位、Amazon アソシエイトのレポート。**亀好きさんの手元にしかないため、まず共有してもらう** |
| **② トリガー判定** | `docs/operations/DECISION_RULE.md` の判断ツリーで P0 / P1 / P2 を決める。推測で直さない |
| **③ 週最大3ページ** | トリガーが立ったページだけを改善する。`docs/operations/WEEKLY_REVIEW.md` の手順に従う |
| **参考** | 実際に発生した Amazon 経由の購入が1件ある。その導線（どのページ → どの商品）が分かれば、①のトリガーとして最も強い |
