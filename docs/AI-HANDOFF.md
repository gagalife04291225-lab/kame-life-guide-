...
| サイトファイルの状態 | 本PRは `species/*.html` 51ファイルの**甲長表記のみ**の変更（10年後の文・meta・stat・サイズ感・env-card・比較表の甲長列）。data / CSS / JS / 画像は無変更 |
...
### 甲長表記を `max_shell_length_cm`（正本）へそろえた — **超過 0**（2026-09-18 / 本PR）— **再調査しない**
...
当初は「10年後」の文だけが対象だったが、同じページの別の場所に同種の超過が残ると
**ページ内で矛盾する**ため、Owner 指示（2026-09-18「まとめて直して」）により甲長表記を全面的にそろえた。

- **対象**: 10年後の文 7件 ／ 自種の甲長表記（meta description・`stat-label`・サイズ感の
  `lp-block-title`・env-card「大きさ」・チェックリスト行・比較表の**自種行の甲長列**）
  ／ 比較表の**他種行の甲長列** 60件。合計 **51ファイル / 151行**。
- **下限の扱い**: 既存の下限は原則そのまま残し、上限だけを `floor(正本max)` へ下げた。
  下限が上限以上になる種だけ、正本 note に根拠がある値へ置換した
  （ホオジロクロガメ 17／アカスジヤマガメ系 18／スペングラーヤマガメ 7）。
  根拠が無いスッポン2種は**下限を作らず「25cm級」**とした。
- ケヅメリクガメの比較表セルは「60〜90cm超」→「60〜86cm」とし、**「超」を外した**
  （正本 max 86cm を超える読みになるため）。これがサイズ表記以外で変わった唯一の箇所。
- **意図的に触れていない2件**（誤検出ではなく仕様）:
  `western-hermann-tortoise` の「ヒガシ亜種（メス19〜23cm・最大28cm級）」は**別種ページの説明**。
  `pink-bellied-side-necked-turtle` の比較表にある「オーストラリアナガクビガメ（近日公開予定）20〜28cm」は
  **未掲載種**で正本が無い。
- 実測: 正本 max を超える甲長表記 **0**（自種・他種行とも）。
  `check_compare_enclosure.py` 矛盾0 / `check_enclosure_sync.py` 不一致0（いずれも着手前と同値）。
  変更95行のうちサイズ表記以外の差分は上記ケヅメ2件のみ（機械照合）。
...
`MINA_MASTER` の**保存場所と参照方法だけ**を整理した。3層の役割分担（fixed / image / template）と FIXED CORE の人物設定は変更していない。

| 項目 | 内容 |
|---|---|
| canonical path | `brand/assets/mina/mina-master.png` |
| 正本の枚数 | 1枚のみ。派生画像は MASTER ではない |
| 差し替え | Owner の明示指示があるときだけ。頻繁に差し替えない |
| 不在時 | 人物固定が必要な生成を開始しない。Owner へ正本の配置を求める |
| 公開範囲 | 本 repo は **public** かつ GitHub Pages 配信ソース（実測）。置いたファイルは公開される。公開してよいオリジナルAIキャラクター画像に限る |

**旧方針「MASTER 画像を repo に保存しない／毎回 Owner が添付する」は SUPERSEDED。**
旧前提の記述は repo 内に1箇所（`brand/mina-fixed-rules.md`）しかなく、そこを置換して残存 0 件にした。

**正本の画像ファイル自体は未配置。** 本作業環境に承認済みの MASTER 画像が存在せず、
ダミー画像の作成と新規生成はいずれも禁止のため、ルールとディレクトリのみ整備した。
...
PR #168 で確定した役割分担（`brand/mina-fixed-rules.md` = 人物の正本 / `brand/mina-image-rules.md` = 生成運用）を維持したまま、「人物は近いが毎回同じ顔角度・同じ距離・同じポーズで、商品だけ差し替えたように見える」問題へ対処した。

| 項目 | 内容 | 置き場所 |
|---|---|---|
| 「固定」の定義 | 固定するのは**人物であって構図ではない**と明文化。同角度・同距離・同表情の反復は人物同一性を守っていても不合格 | fixed §1 |
| 同角度量産の禁止 | 7項目の禁止と、顔向き5種から**3種類以上**を使う分散規則 | image-rules |
| カメラ距離分散 | 9種の母集合。全カットに顔を入れる必要はなく、証拠カットは手元を主役にしてよい | image-rules |
| 視線・顔向き・動作の整合 | 行動別の視線先7種を表で定義。両目の視線延長線による目視QCを必須化 | image-rules |
| カメラ目線の限定 | 「商品を視聴者へ紹介」「CTA・リアクション」の2場面のみ許可 | image-rules |
| 背景ローテーション | 母集合を9種へ拡張（定義）／回し方と動画内 continuity（運用） | fixed §4 / image-rules |
| 商品サイズ維持 | 縮尺をカット間で変えない。**実寸の正本は商品レコード側**とし二重管理しない | image-rules |
| SHOT PLAN | 生成前の必須ショット表。隣接CUTで最低2要素を変更、ただし空間・服・髪・商品サイズは維持 | image-rules |
| 生成後QC | IDENTITY / SHOT / GAZE / ACTION / PRODUCT / HANDS / BACKGROUND / ARTIFACT の8分類 | image-rules |

**人物設定は正本 1 ファイルにのみ存在し、生成運用側へ複製していない**（機械検査で確認）。
画像生成・商品再選定・動画生成は本PRのスコープ外。
...
- **甲長表記の正本同期は本PRで完了**（COMPLETED 参照）。超過 0 を実測済み。**再監査しない。**
...
### env-card のラベルを容器種別へそろえる — 7ページ

**対象**（2026-09-18 実測）: `ls-lead` が「ケージ」なのに env-card のラベルが `必要水槽` になっているページ。
`annulated-wood-turtle` / `brown-wood-turtle-manni` / `guerrero-wood-turtle` /
`nicaragua-wood-turtle` / `painted-wood-turtle` / `sonora-wood-turtle` / `wood-turtle`（いずれも陸生寄りのヤマガメ）。
**Scope**: `env-label">必要水槽</div>` の**ラベル文字列のみ**。表示値（`env-value`）は本PRで正本へそろえ済みなので触らない。
**変更禁止**: 甲長・容器サイズの値そのもの（正本同期済み）／`data/species-master.json`／
水棲種の `必要水槽` ラベル（正しい）／ASIN・Amazon ID。
**完了条件**: 7ページすべてで `ls-lead` の容器種別と env-card のラベルが一致すること。
サイト全体のラベル内訳は現在 水槽40 / ケージ23 / 環境20 / スペース1。
**「環境」「スペース」への統一はしない**（表記ゆれの是正は別途 Owner 判断）。
