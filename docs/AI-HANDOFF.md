...
| 項目 | 値 |
|------|-----|
| 基準 | `origin/main` = **c4f2542**（PR #98 = 公開UIの種数118統一の merge commit まで反映済み） |
| サイトファイルの状態 | **PR #98 = `6b611d6`** が最後。**merge 待ち = 本PR（species-list.html のデザイン実装1）** |
| 確認方法 | `git log --oneline -1 origin/main` で**実測する**。表の値は下記の理由で常に1〜2 commit 遅れる |
| 最終更新日 | 2026-08-28 |
| 作業ブランチ | `claude/kame-life-species-list-impl1` |
| 作業ツリー | clean（`origin/main` ＋ 本PRの変更のみ。変更は `species-list.html` と `docs/AI-HANDOFF.md`） |
| 本PRの内訳 | ① デザイン実装1（`223481a`）／② 法規制表示の回帰修正（本コミット） |
...
| 実装1 | 監査結果の実装 | 本PR | `species-list.html` **1ファイル・+287/−119行**。下表のとおり |
...
- **カードの表示情報は「実写真 / 和名 / 学名 / 難易度 / 甲長（最大◯cm）」の5点＋法規制ラベルで確定。**
  ...
  `legal` フラグを持つ14種にだけ出る法規制ラベルを本PRの2コミット目で入れた**（下の「法規制表示の回帰修正」）。
...
| 回帰修正 | 実装1で消えた法規制表示の復旧 | 本PRの2コミット目 | `species-list.html` 1ファイル。`legal` を持つ**14種だけ**にラベルを出す |
...
| 対象 | 内容 |
|------|------|
| **一覧の掲載区分（通常115 / 参考3）** | 監査で設計まで確定済み（→ `NEXT`）。実装するかの判断だけが残っている。**通常一覧115種＋参考掲載3種**（エジプトリクガメ・マダガスカルクモノスガメ・モエギハコガメ）。掲載総数は118のままなので**公開UIの「118種」表記は変更不要**。`shindan/species.js` は118件全件維持し、`availability` / `match` / `score` は触らない（診断影響ゼロ）。掲載区分の正本は `tools/taxonomy.js` に置き、generator が `taxonomy-data` へ焼き込む |
...
| ID | 内容 |
|----|------|
| ~~N29~~ | **解消済み（本PR）**。個別ページを持たない6種のカードに「飼育ガイドへ」を表示し、遷移先が種ページでないことを明示した |
...
### ② species-list.html の掲載区分 — 通常一覧115種 ＋ 参考掲載3種

監査で設計は確定済み（→ `COMPLETED` の8種判定表）。**再監査・再判定しない。**
本PRの回帰修正で法規制ラベルは戻したが、**参考データが通常一覧に混ざっている状態は残っている。**

| 項目 | 内容 |
|------|------|
| **やること** | 一覧を「通常一覧115種」と「参考掲載3種」に分ける。参考掲載は**エジプトリクガメ / マダガスカルクモノスガメ / モエギハコガメ**の3種で、一覧末尾の独立セクションに置く。**検索対象からは外さない**（ヒットしたら参考セクションを見せる） |
| **なぜこれが次か** | 一覧の目的は「日本で現実的に飼育候補にできる種を選ぶ」こと。入手性を確認できない3種が通常グリッドに並んでいると、選択肢として誤認される |
| **掲載区分の正本** | `tools/taxonomy.js`。generator が `BEGIN:taxonomy-data` へ焼き込む。**生成領域を手で編集しない** |
| **件数の扱い** | 件数バーは「118種（通常一覧 115 / 参考掲載 3）」。**総数118は減らないので公開UIの「118種」表記は変更不要。112 という数字は出さない** |
| **カテゴリ件数** | リクガメ 27→25 ／ ヤマガメ・ハコガメ 22→21 ／ 半水棲18・水棲29・汽水6・スッポン曲頸16 は不変。`gen-category-ui.js` 側で対応する |
| **noscript** | `BEGIN:species-index` は SEO クロール用リンクなので**118件のまま維持**し、見出しだけ参考掲載を分ける |
| **テキサスチズガメ** | HOLD。除外根拠が無いので**通常一覧に残す**。今回動かさない |
| **対象** | `species-list.html` ／ `tools/taxonomy.js` ／ `tools/gen-species-list.js` ／ `tools/gen-category-ui.js` |
| **変更禁止** | `shindan/species.js`（118件全件維持・`availability` / `match` / `score` を触らない）／ 診断ロジック ／ `title`・`meta`・OG・twitter・JSON-LD（H8）／ `index.html` ／ 「118種」表記 ／ 生成領域の手編集 ／ N22・N23・N24・N27・N28・U-D・H3・H8・H9 |
| **完了条件** | 通常一覧115・参考掲載3・合計118が画面と正本で一致／ 参考掲載3種が検索でヒットする／ 390px の全長が 21,000px を超えない／ 2列維持・横スクロール0・sticky維持・JSエラー0 ／ generator 4本 `--check` 差分0 ／ SEO層・JSON-LD 差分0 ／ `docs/AI-HANDOFF.md` を同期 |

**着手は本PR（#99）の merge 後。** 本PRに混ぜない。

**「暮らしから選ぶ」6大分類の写真タイル（旧・実装2）は、掲載区分の実装後に回す。**
先に載せる種を確定させないと、タイルの種数表示が二度手間になるため。
