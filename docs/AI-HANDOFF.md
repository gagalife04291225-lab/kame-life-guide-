...
| 項目 | 値 |
|------|-----|
| 基準 | **PR #71**（H7 解消 — スコア表の軸を表と導入文で同時に統一）を含む `origin/main` |
...
### E-2 `shindan/index.html` の関連リンクカード15行 — 可視ラベルのみ

**なぜ今これか**: 残る HOLD（H5 / H6）と U1 / U2 は**すべて亀好きさんの判断待ち**で、
Claude Code 側だけでは進められない。`N3` の残り62行も方針未定。
そのなかで **`shindan/index.html` の関連リンクカードだけは構造が完全に均一**で、
機械抽出でき、置換先の語彙も**同じファイル内に既にある**（PR #62 で確定した
`はじめての亀 TOP3`）。B-2（PR #53）で確立した「CTA・関連リンク・ナビの可視ラベル」の
続きにあたり、判断待ちを1つも増やさずに閉じられる。

| 項目 | 内容 |
|------|------|
| **対象** | `shindan/index.html` の診断結果に出る関連リンクカード **15行**（`{ name: ..., desc: ..., url: ... }` の形）。行番号は `3079` `3082` `3084` `3087` `3089` `3099` `3104` `3136` `3142` `3152` `3165` `3167` `3177` `3180` `3185`。※着手時に現mainから再抽出し、**15行と一致しなければ変更せず STOP して差異を報告** |
| **異なり文字列は4種だけ** | `name: '初心者向け亀'`（10件）／`name: '初心者向け亀比較'`（1件）／`desc: '初心者おすすめ比較'`（3件）／`desc: '丈夫で初心者向き'`（3件）。**同じ文字列は同じ語に揃える** |
| **Scope** | `name` と `desc` の**可視ラベルのみ**を置換する。`name: '初心者向け亀'` は、**同じファイルに既にある PR #62 確定の `はじめての亀` に合わせる**のが基本。`desc` は既存の確定語彙（`はじめて飼う方` / `はじめての1頭` / `はじめてでも`）から選び、**新語を作らない**。文の長さはカード表示に収まる範囲を保つ（現行は最長10文字前後） |
| **絶対に変更禁止** | `url:`（`BASE+'guides/beginner-best-turtles.html'` / `BASE+'compare/hermann-vs-greek.html'` 等）／オブジェクトのキー名（`name` / `desc` / `url`）／配列の順序・件数／`BASE` 変数 |
| **同ファイル内で触らないもの** | `shindan/index.html` に残る他の「初心者」14件 — コメント8件（非表示）／`reason:` 2件（`:2668` `:2860`）／バッジ配列2件（`:1394` `:1400` の `'🎯 初心者向き'`）／生成文2件（`:1450` `:1567`）。**いずれも別工程。今回は触らない** |
| **その他の変更禁止** | H5 / H6 の9件（判断待ち）／`shindan/species.js`・`routes.js`・`equipment.js`／診断ロジック・採点・ルート定義／difficulty の値・★・色・アイコン／確定済み文言（A-1 `飼育難易度` / A-4 / A-5 / B・C・D・F の全確定文 / annainin の `はじめての方向け` 系4箇所 / PR #70 の compare 6件 / PR #71 のスコア表5行）／`title`・`meta`・OG・canonical／`href`・URL・slug／GA4イベント名・パラメータ／`data-*`／`id`・`class`／taxonomy／写真／CITES／学名・和名 |
| **開始前ゲート** | 15行それぞれで ①`url:` の値が置換対象文字列を含まないこと ②GA4 パラメータに使われていないこと ③JSON-LD複製がないこと を機械確認する |
| **完了条件** | ①15行の置換完了（同じ文字列は同じ語）②`url:` に差分0 ③診断ツールを実描画し、**5ルートすべてで結果画面の関連リンクカードの遷移先（href）が変更前と一致**すること ④カードの表示件数・順序が変更前と一致 ⑤Scope Lock 対象に意図外差分0 ⑥JSエラー増加0 ⑦generator 4本 `--check` 差分0 ⑧commit → push → PR → merge → main反映確認 ⑨本ファイルの更新 |
