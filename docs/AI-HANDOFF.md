...
|----|----------------|-----------------|
| **B11 カメ動画の観測データ（Instagram 研究の再開条件）** | **観測できた実例10本分の記録**。取得経路は次のいずれか — ①亀好きさんが Instagram を自分の目で見て `experiments/ig-study/OBSERVATION-TEMPLATE.md` に10本記録する（完全合法・鍵不要）／②**YouTube Data API v3 の無料キー**を取得し repo secret へ登録する（ToS準拠・再生数が機械で取れる） | 2026-09-03 実測により **Instagram は自動観測不可で STOP**。`instagram.com/robots.txt` は `User-agent: *` に対し **`Disallow: /`**（サイト全体）。Graph API は Instagram Business ID とトークンが必須で **BLOCKED_API_KEY**。**YouTube 公式RSS も `/feeds/videos.xml` が Disallow** のため代替にならない。<br>記録が10本そろった時点で、伸びている群と伸びていない群の**差分**を取り、KAME SHORTS PLAYBOOK（①冒頭 ②行動 ③構図 ④カットテンポ ⑤15〜30秒テンプレ ⑥禁止演出 ⑦次の実験）へ変換する。**10本未満で「パターン」と書かない。** |
