...
| 項目 | 値 |
|------|-----|
| 基準 | `origin/main` = **75b002b**（PR #148 merge・2026-09-07 実測値） |
...
指示では「実装前に各公式料金ページを確認し、現在の金額と一致するものだけ使う」とされたが、
**本セッションからは候補5病院の公式サイトすべてに到達できなかった**
（`curl` / `WebFetch` とも `EGRESS_BLOCKED` / HTTP 000）。
到達不能であり **NOT FOUND ではない**。

- 唯一 `WebSearch` 経由で、みわエキゾチック動物病院の公開料金として
  初診料 3,800円 / 再診 1,300円〜 / レントゲン 5,000円〜 という値を確認でき、
  Owner 提示のレンジ内に収まることは照合できた（**検索結果の要約であり公式ページ原文ではない**）
- そのため **特定の病院名と個別金額を紐づけた記載はしていない**。
  「料金を公開している爬虫類・エキゾチック対応病院の料金表をもとにした目安」という
  書き方に留め、誤った金額を実在の病院に帰属させないようにした

**確定した結論**: 掲載値は Owner 提示のレンジであり、公式ページ原文の再確認は未実施。
**公開後に Owner が各病院の料金ページで再確認すること。**
