...
- **カードの表示情報は「実写真 / 和名 / 学名 / 難易度 / 甲長（最大◯cm）」の5点で確定。**
  CITES と 条件付特定外来のバッジは一覧から外し、詳細ページで見せる（`legalBadge()` は削除済み）。
...
### ② species-list.html デザイン実装2 — 「暮らしから選ぶ」写真タイルの追加

実装1（本PR）で一覧そのものは軽くなった。**再実装・再測定しない。**
監査で設計した新構成のうち、**まだ入っていないのは「⑤ 暮らしから選ぶ（6大分類の写真タイル）」だけ**。

| 項目 | 内容 |
|------|------|
| **やること** | 操作バーと一覧の間に、6大分類の写真タイル（2列×3行・各タイルに種数 27/22/18/29/6/16）を置く。タップでその分類に絞り込む（既存の `#hab-btns` の状態を動かすだけで、フィルタのロジックは足さない） |
| **なぜこれが次か** | 現在 6大分類は操作バーの横スクロールチップだけで、写真で選ぶ入口がない。図鑑としての第一印象はここで決まる |
| **タイルの写真** | **新規撮影・生成はしない。** 既存の `assets/species-photos/*.webp` から各分類の代表種1枚を流用する。どの種を代表にするかは実装時に決めて報告する |
| **対象** | `species-list.html` のみ |
| **変更禁止** | 生成領域4つ（`hab-buttons` / `taxonomy-data` / `wamei-alias` / `species-index`）／ `title`・`meta`・OG・twitter・JSON-LD（H8）／ `shindan/species.js`・`equipment.js`・`CAT_OVERRIDE` の和名3点連結 ／ `index.html` ／ 診断ロジック ／ N22・N23・N24・N27・N28・U-D・H3・H8・H9 |
| **完了条件** | 390px の全長が **21,000px を超えない**（タイル追加ぶんの増加を抑える）／ タイルから絞り込みが効く／ 118種・6分類の件数が正本と一致／ 検索・絞り込み・並び替えが従来どおり／ JSエラー0 ／ 横スクロールなし ／ generator 4本 `--check` 差分0 ／ SEO層・JSON-LD 差分0 ／ `docs/AI-HANDOFF.md` を同期 |
