...
const CATEGORY_GUARDS = {
  ...
  // '砂' 'ソイル' は大磯砂・デザートベース・プランテーションソイル等、
  // 商品名に「床材/サンド」を含まない国内表記の床材を拾うため
  substrate:        ['床材', '土', 'サンド', 'マット', 'バーク', 'マルチ', '砂', 'ソイル'],
  ...
  food:             ['フード', '餌', 'エサ', 'ペレット'],
  // '乳酸菌' は爬虫類用プロバイオティクスの国内表記
  supplements:      ['カルシウム', 'サプリ', 'ビタミン', '乳酸菌'],
  thermometer:      ['温度計', 'サーモ', '温湿度'],
  // 2026-08-04 追加。lighting_fixture / water_care は新設カテゴリ。
  // water_dish はカテゴリとしては存在していたがガードが無く、
  // guards.length === 0 で全候補が素通りする状態だった
  lighting_fixture: ['ソケット', 'スタンド', 'ドーム', '灯具', 'クリップ', 'ランプ'],
  water_care:       ['カルキ', '水質', '塩素', '中和', '水つくり', 'コンディショナー'],
  water_dish:       ['水入れ', 'ディッシュ', 'ボウル', '水容器', '水皿'],
};
