...
## 【別プロジェクト】AI TikTok Company（設計完了・長期記憶）

> このリポジトリのブランチ `claude/ai-tiktok-oss-architecture-u6xkun` 配下で進行している
> **別プロジェクト**（カメライフガイドとは別物）。AI Company OS の正式プロジェクトの1つ。

- **設計は完了済み。ゼロから設計をやり直さず、既存設計を標準仕様として利用すること。**
- 長期記憶・標準仕様の索引: **`docs/ai-tiktok-company/PROJECT_MEMORY.md` を必ず最初に読む。**
- 設計本体: `docs/ai-tiktok-company/` の Phase 0 / 0.5 / 1 / 1.5 / 1.6。
- 開始位置 = **Phase 2（MVP実装・検証）**。設計の再提案ではなく実装への落とし込みを優先。
- 決定済みアーキ: Claude Code + GitHub + GitHub Actions + MCP + ComfyUI。
- 品質判断の絶対順序（不変）: 正確性 ＞ 信頼 ＞ ブランド ＞ 品質 ＞ 再生数 ＞ 利益。
- 継続ルール: ①既存設計との整合性最優先 ②憲法/運用基盤/AI社員仕様と矛盾を作らない
  ③実測優先・推測で仕様変更しない ④変更時は理由/影響範囲/関連フェーズを明示（憲法§⑫改訂手順）。
