...
指摘12件のうち10件は差し替え済み（PR #34・2026-08-22）。未完了は次の2件のみ:

| slug | 理由 | 状態 |
|------|------|------|
| `pancake-tortoise` | 岩の隙間に挟まり、暗くピントも合っていない | 判断待ち。iNaturalist に商用利用可の代替候補が現行1件しか見つかっていない |
| `albino-chinese-softshell` | 通常個体に見える。観察ID 245665 をスッポンのページと共用 | 検索上位にアルビノ個体の商用利用可写真が確認できず未確保 |
