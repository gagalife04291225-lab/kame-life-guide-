...
| 節 | 扱い |
|----|------|
| `CURRENT_BASE` | 基準にする main commit |
| `COMPLETED` | **固定入力。再調査・再監査・再実装しない** |
...
**作業終了時は同じPRの中で `docs/AI-HANDOFF.md` を更新する。**
...
## 公開語彙ルール — 「初心者」（恒久ルール・2026-08 亀好きさん承認）

これは恒久ルール。§サイト名称・ブランディング憲法と同じ扱いで、他の記述と矛盾する場合は優先する。

- **公開表示では「初心者」を新規使用しない。**
- 文脈に応じて確定語彙を使う。**新語を作らない。**
  「はじめての方向け」／「はじめて飼う方」／「はじめてでも」／「はじめての1頭」
- **非表示・機能文字列は変更しない。** 内部互換・分類キーワード・URL・slug・
  `value` 属性・GA4 のイベント名とパラメータ・コメントは対象外
  （例: `?diff=初心者向け`／`<option value="beginner">`／
  `js/annainin.js` の `INTENT_RULES`）。
- **「入門」は difficulty 正本値なので、一律の置換語として使わない。**
- `title` / `meta` / OG / JSON-LD は SEO と不可分。
  運営フェーズの `docs/operations/DECISION_RULE.md` に従い、
  **GSC/GA4 の実測がトリガーを引いたページのみ**扱う。
...
> **運用実体は [`docs/AI-HANDOFF.md`](docs/AI-HANDOFF.md)。**
> 「完了済み・確定済み」が具体的に何を指すかは同ファイルの
> `COMPLETED` / `FIXED_FACTS` が定める。本節はその判断基準を述べる。
...
- **HOLD 4件は再探索禁止。**（2026-08-24 の外部ソース完全調査で1件を実装・4件を再確定）
  ...
  **能動的な再探索・定期チェック・権利者への個別連絡はいずれも行わない**
  （下記「再開条件」を参照）。
...
### 再開条件（2026-08-24 亀好きさん決定・恒久）

**HOLD 4件はここで完全にクローズする。以下を恒久方針とする。**

- **外部の撮影者・研究者・権利者への個別連絡は行わない。**
  Torsten Blanck 本人への画像提供依頼を含め、個別の許諾交渉はしない。
- **定期探索・定期監視は行わない。** 週次・月次・四半期のいずれでも
  HOLD 案件の写真を探しに行かない。
- **再開できるのは、将来まったく別の作業をしている最中に、
  公開された利用可能な新規写真をたまたま発見した場合のみ。**
  その場合も、発見をきっかけに探索を再開するのであって、
  探しに行く作業を新たに立てることはしない。

したがって、HOLD 4件は「いつか写真が見つかれば実装できる状態で凍結」であり、
**能動的に追いかけるタスクとしては存在しない。**
