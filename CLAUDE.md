...
### 既存写真の全数監査・差し替え記録（完了・アーカイブ済み）

2026-08-22 に公開中の生体写真123枚を全数目視し、基準抵触4件を差し替えた。
2026-08-23 に出典URL重複10グループ24ページを一次データで照合し、16件を差し替え・2件を
KEEP・5件を HOLD として監査を完了した。判定表・出典表・作者・ライセンスの全記録は
[`docs/archive/COMPLETED-PROJECTS.md`](docs/archive/COMPLETED-PROJECTS.md) にある。

**この監査は NO-REWORK GATE の対象。全数監査・出典照合を再実行しない。**
HOLD 案件の扱いは本ファイルの「再開条件」節に従う。
...
## 亜種PROJECT 完了記録（アーカイブ済み）

Phase B（候補11件）・Phase C（候補56件・IMPLEMENT 5 / HOLD 3 / DROP 48）の判定表、
title 重複修正の記録、HOLD案件の外部ソース完全調査、HOLD 4件の写真素材の実測照合表は
[`docs/archive/COMPLETED-PROJECTS.md`](docs/archive/COMPLETED-PROJECTS.md) に移した。

**いずれも NO-REWORK GATE の対象。再監査・再判定・再探索は行わない。**
正式クローズの方針と再開条件は下記の各節に残してある。
...
- **HOLD 4件は再探索禁止。**（2026-08-24 の外部ソース完全調査で1件を実装・4件を再確定）
  ...
  **能動的な再探索・定期チェック・権利者への個別連絡はいずれも行わない**
  ...
  （記録は `docs/archive/COMPLETED-PROJECTS.md`）。**この照合をやり直さない。**
